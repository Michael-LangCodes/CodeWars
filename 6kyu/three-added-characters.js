// Given two strings, the first being a random string and the second being the same as the first, but with three added characters somewhere in the string (three same characters),

// Write a function that returns the added character

// You can assume that string2 will aways be larger than string1, and there will always be three added characters in string2.

// Examples
// string1 = "hello"
// string2 = "aaahello"
// => 'a'

// string1 = "abcde"
// string2 = "2db2a2ec"
// => '2'

// string1 = "aabbcc"
// string2 = "aacccbbcc"
// => 'c'

//My Solution
function addedChar(s1, s2){
    let checker_s1 = {}
    let checker_s2 = {}
    s1.split('').forEach(item => {
    checker_s1[item] = (checker_s1[item] || 0) + 1;
  });  
  s2.split('').forEach(item => {
    checker_s2[item] = (checker_s2[item] || 0) + 1;
  }); 
  
  for (const [key, value] of Object.entries(checker_s2)) {
    if (checker_s2[key] !== checker_s1[key] ){
      return key
    }
}
}
