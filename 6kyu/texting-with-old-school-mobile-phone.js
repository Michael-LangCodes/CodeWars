...
function sendMessage(message) {
  ...
  function findKey(char) {
    for (let key in keys) {
      const idx = keys[key].indexOf(char);
      if (idx !== -1) {
        return { key, presses: key.repeat(idx + 1) };
      }
    }
    return null;
  }
  ...
  for (let char of message) {
    ...
    if (/[0-9*#]/.test(char)) {
      ...
      // WAIT check (must happen BEFORE output)
      ...
      prevKey = key;        // keep key for future comparisons
      prevWasHold = true;   // mark as hold
      continue;
    }
    ...
    // CASE SWITCHING (only for letters)
    if (isLetter) {
      const neededCase = char === lowerChar ? 'lower' : 'upper';

      if (neededCase !== currentCase) {
        result += '#';
        currentCase = neededCase;

        // reset wait conditions (case switch cancels waiting)
        prevKey = null;
        prevWasHold = false;
      }
    }

    const found = findKey(lowerChar);
    if (!found) continue;

    const { key, presses } = found;
