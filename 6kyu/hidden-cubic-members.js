...
function isSumOfCubes(s){
  ...
  const numCheck = (input) => /\d/.test(input)
  for (let i = 0; i < splitS.length; i++){
    let iterator = 0
    let numToCheck = ''
    if(numCheck(splitS[i])){
      numToCheck += splitS[i]
      if(numCheck(splitS[i+1])){
        numToCheck += splitS[i+1]
        iterator ++
        if(numCheck(splitS[i+2])){
          numToCheck += splitS[i+2]
          iterator ++
        }
      }
      let splitNumToCheck = numToCheck.split('')
      let totalCheck = splitNumToCheck.reduce((acc,curr)=>{
        curr = Number(curr)
        acc += curr**3
        return acc
      },0)
      if(totalCheck == numToCheck){
        if (output === ''){
          if(numToCheck === '000' || numToCheck ==='00'){
            output += '0'
          }
          else{output += numToCheck}
        }
        else{
          if(numToCheck === '000' || numToCheck ==='00'){
            output += ' 0'
        ...
        outputTotal += totalCheck
      }
      i += iterator
  }
    }
  if(output !== ''){
    output += ` ${outputTotal} Lucky`
  }else{
    output += 'Unlucky'
  }
  return output
